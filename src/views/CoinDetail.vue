...
import api from '../api'
    export default {
        ...
        computed:{
            min(){
                return Math.min(...this.history.map(h => h.priceUsd))
            },
            max(){
                return Math.max(...this.history.map(h => h.priceUsd))
            },
            avg(){
                return (this.history.reduce((a,b)=> a + Number(b.priceUsd), 0)/this.history.length)
            }
        },
        methods:{
            getWebSite(excange){
                return api.getExchange(excange.exchangeId.replace(' ', '-').replace('(', '').replace(')','')).then(res => {
                    this.$set(excange, 'url', res.data.data.exchangeUrl)
                })
            },
            getCoin(){
